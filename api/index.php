...
<body>
    ...
    <div class="container">
        ...
        <div id="tab-rc-content" class="terminal-box" style="display: none;">
            ...
            <div class="input-group">
                ...
                <input type="text" id="rcInput" class="input-field" placeholder="Enter RC number (e.g., dl9c0001)" value="" onkeypress="if(event.key==='Enter') fetchRCInfo()">
            </div>
        ...
        <div style="text-align: center; margin-top: 20px; color: rgba(0,255,0,0.5); font-size: 0.8em;">
            <i class="fas fa-crown"></i> Developed with <i class="fas fa-heart" style="color: #0f0;"></i> by Keerthu Poojary © 2026
        </div>
    </div>
    ...
    <script>
        ...
        function displayNumberResults(data, resultsDiv) {
            ...
            let html = '<div class="stats-mini"><span><i class="fas fa-database"></i> TOTAL: ' + results.length + '</span><span><i class="fas fa-user"></i> PRIMARY: ' + primary.length + '</span><span><i class="fas fa-users"></i> ASSOCIATED: ' + associated.length + '</span><span><i class="fas fa-clock"></i> TIME: ' + data.result.search_time + '</span></div>';
            if (primary.length > 0) {
        ...
        function createNumberCard(result) {
            ...
            fields.forEach(field => {
                if (field.value && field.value.toString().trim() !== '') {
                    html += '<div class="card-field"><div class="field-label"><i class="fas ' + field.icon + '"></i> ' + field.label + ':</div><div class="field-value">' + escapeHtml(field.value) + '<button class="copy-btn" onclick="copyToClipboard(\'' + escapeHtml(field.value) + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
                }
            });
            html += '</div>';
            return html;
        ...
        async function fetchRCInfo() {
            ...
            try {
                ...
                const data = await response.json();
                if (data.error) {
                    showError(resultsDiv, 'RC NOT FOUND');
                } else {
        ...
        function displayRCResults(data, resultsDiv) {
            ...
            if (vehicleData["registration_identity_matrix"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-id-card"></i> REGISTRATION IDENTITY</div>';
                Object.entries(vehicleData["registration_identity_matrix"]).forEach(([key, value]) => {
                    if (!key.includes('seal') && !key.includes('auth') && !key.includes('token')) {
                        html += '<div class="card-field"><div class="field-label">' + formatLabel(key) + ':</div><div class="field-value">' + (escapeHtml(value) || 'N/A') + '<button class="copy-btn" onclick="copyToClipboard(\'' + escapeHtml(value) + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
                    }
                });
                html += '</div>';
            }
            if (vehicleData["ownership_profile_analytics"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-user"></i> OWNERSHIP DETAILS</div>';
            ...
            if (vehicleData["technical_structural_blueprint"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-cog"></i> TECHNICAL SPECIFICATIONS</div>';
            ...
            if (vehicleData["insurance_security_audit_report"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-shield-alt"></i> INSURANCE DETAILS</div>';
            ...
            if (vehicleData["lifecycle_compliance_timeline"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-calendar-alt"></i> LIFECYCLE TIMELINE</div>';
            ...
            if (vehicleData["financial_legal_encumbrance_vault"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-gavel"></i> FINANCIAL & LEGAL</div>';
            ...
            if (vehicleData["ai_performance_valuation_matrix"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-chart-line"></i> PERFORMANCE MATRIX</div>';
            ...
            if (vehicleData["regional_transport_intelligence_grid"]) {
                html += '<div class="hacker-card"><div class="card-title"><i class="fas fa-map-marker-alt"></i> REGIONAL INFO</div>';
        ...
        function displayIGResults(profile, resultsDiv) {
            let html = '<div class="hacker-card">';
            html += '<div class="ig-profile"><div class="ig-avatar"><img src="' + (profile.profile_pic_url_hd || 'https://via.placeholder.com/100') + '" alt="Profile" onerror="this.src=\'https://via.placeholder.com/100\'"></div><div style="flex:1;"><div style="display:flex; align-items:center; gap:10px; margin-bottom:10px; flex-wrap:wrap;"><span style="color:#0f0; font-size:1.3em;">@' + profile.username + '</span>' + (profile.is_verified ? '<span class="badge" style="background:rgba(0,255,0,0.2); border-color:#0f0;"><i class="fas fa-check-circle"></i> VERIFIED</span>' : '') + (profile.is_private ? '<span class="badge" style="background:rgba(255,255,0,0.2); border-color:#ff0; color:#ff0;"><i class="fas fa-lock"></i> PRIVATE</span>' : '') + '</div><div style="color:#fff; font-size:1.1em; margin-bottom:5px;">' + escapeHtml(profile.full_name) + '</div><div style="color:rgba(0,255,0,0.7); font-size:0.9em; margin-bottom:10px;">' + escapeHtml(profile.biography) + '</div><div class="ig-stats"><div class="stat-box"><div class="stat-number">' + (profile.posts || 0) + '</div><div class="stat-label">POSTS</div></div><div class="stat-box"><div class="stat-number">' + (profile.followers || 0) + '</div><div class="stat-label">FOLLOWERS</div></div><div class="stat-box"><div class="stat-number">' + (profile.following || 0) + '</div><div class="stat-label">FOLLOWING</div></div></div></div></div>';
            ...
            details.forEach(d => {
                if (d.value) {
                    html += '<div class="card-field"><div class="field-label"><i class="fas ' + d.icon + '"></i> ' + d.label + ':</div><div class="field-value">' + escapeHtml(d.value) + '<button class="copy-btn" onclick="copyToClipboard(\'' + escapeHtml(d.value) + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
                }
            });
            if (profile.external_url) {
                html += '<div class="card-field"><div class="field-label"><i class="fas fa-link"></i> WEBSITE:</div><div class="field-value"><a href="' + profile.external_url + '" target="_blank" style="color:#0f0;">' + profile.external_url + '</a><button class="copy-btn" onclick="copyToClipboard(\'' + profile.external_url + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
            }
            html += '</div>';
            resultsDiv.innerHTML = html;
        }
        ...
        function displayAdharResults(data, resultsDiv) {
            ...
            let html = '<div class="stats-mini"><span><i class="fas fa-database"></i> TOTAL: ' + (data.total_found || results.length) + '</span><span><i class="fas fa-id-card"></i> AADHAR: ' + targetAadhar + '</span></div>';
            if (results.length > 0) {
        ...
        function createAadharRecordCard(record, index, isDuplicate = false) {
            let html = '<div class="hacker-card"><div class="card-title"><i class="fas fa-mobile-alt"></i> RECORD #' + index + (isDuplicate ? '<span class="badge duplicate"><i class="fas fa-copy"></i> DUPLICATE MOBILE</span>' : '') + '</div>';
            const fields = [
            ...
            fields.forEach(field => {
                if (field.value && field.value.toString().trim() !== '') {
                    if (field.label === 'MOBILE' || field.label === 'ALTERNATE') {
                        html += '<div class="card-field"><div class="field-label"><i class="fas ' + field.icon + '"></i> ' + field.label + ':</div><div class="field-value"><span style="color:#0f0; font-weight:bold;">' + escapeHtml(field.value) + '</span><button class="copy-btn" onclick="copyToClipboard(\'' + escapeHtml(field.value) + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
                    } else {
                        html += '<div class="card-field"><div class="field-label"><i class="fas ' + field.icon + '"></i> ' + field.label + ':</div><div class="field-value">' + escapeHtml(field.value) + '<button class="copy-btn" onclick="copyToClipboard(\'' + escapeHtml(field.value) + '\', this)"><i class="fas fa-copy"></i> COPY</button></div></div>';
                    }
                }
